...
extern size_t nthreads;  // the warehouses per shard, --num-threads
extern size_t nshards; // the shards in DB, --num-shards
extern size_t num_erpc_server;
extern size_t shardIndex; // at which the current shard is running (starting from 0), --shard-index
extern std::string cluster; // "localhost", "p1", "p2", "learner"
extern int clusterRole;
extern size_t workload_type;
extern transport::Configuration *config; // shard configuration file
...
extern volatile int runtime_plus;
