...
namespace transport
{
    ShardAddress::ShardAddress(const string &host, const string &port, const int &clusterRole) : host(host), port(port), clusterRole(clusterRole)
    {
    ...
    void Configuration::ParseNewFormat(YAML::Node &config)
    {
        ...
        for (size_t i = 0; i < sites_node.size(); i++)
        {
            ...
            if (site_node["region"])
            {
                site.region = site_node["region"].as<string>();
            }
            else
            {
                site.region = ""; // or a default region like "unknown"
            }
    ...
    ShardAddress Configuration::shard(int idx, int clusterRole) const
    {
        if (is_new_format)
        {
            ...
            if (idx >= 0 && idx < (int)shard_map.size())
            {
                ...
                int replica_idx = 0;
                if (clusterRole == mako::P1_CENTER_INT)
                    replica_idx = 1;
                else if (clusterRole == mako::P2_CENTER_INT)
                    replica_idx = 2;
                else if (clusterRole == mako::LEARNER_CENTER_INT)
                    replica_idx = 3;
    ...
    {
        if (!is_new_format || shard_id >= nshards)
            return nullptr;
